...
library(MARSS);library(xtable);require(broom)
...
for(region in c("ECV","Monterey","Conception")) { # region <- "Conception" 

for (age in c("age_juv","age_A50","age_mat")) { # age <- "age_mat"

load(paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/data/ml_",region,"_",age,".Rdata",sep=''))

# Only keep lingcod in ECV  
if(region!="ECV") {ml<-subset(ml, select = -c(lingcod))}
  
# Transpose the dataset so time series are in columns
dfa <- t(ml)

# Remove species with not enough data
dfa <- data.frame(dfa,sum=NA)
for (xx in 1:length(dfa[,1])) { dfa[xx,"sum"] <- sum(!is.na(dfa[xx,])) }
dfa <- dfa[dfa$sum>3,] # keep at least 4 data points
dfa <- dfa[,-ncol(dfa)]
colnames(dfa) <- 1977:2018
dfa <- as.matrix(dfa)


# Plot the data
jpeg(paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_DFA data.jpeg",sep=''), width=15, height=5, units="in", res=250)
spp = rownames(dfa)
par(mfcol=c(2,4), mar=c(3,4,1.5,0.5), oma=c(0.4,1,1,1))
for(i in spp){
  plot(colnames(dfa),dfa[i,],xlab="",ylab="Length (cm)", bty="L", pch=16, col="blue", type="b")
  title(i)
}
dev.off()
...
cntl.list = list(minit=200, maxit=10000, allow.degen=FALSE)
...
write.csv(model.tbl, file=paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_model table.csv",sep=''))
...
jpeg(paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_DFA trends.jpeg",sep=''), width=8, height=3, units="in", res=250)
...
# Save trends
save(ts.trends, file=paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_trends.Rdata",sep=''))

# Save factor loadings
save(Z.rot, file=paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_loadings.Rdata",sep=''))
...
# plot the fits
jpeg(paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_DFA fits.jpeg",sep=''), width=15, height=5, units="in", res=250)
ylbl = rownames(dfa)
w_ts = seq(dim(dfa)[2])
par(mfcol=c(2,4), mar=c(3,4,1.5,0.5), oma=c(0.4,1,1,1))
for(i in 1:N.ts) {
...
# Run the best model with the added temp covariate 
  ...
  temp1 = t(matrix(temp[colnames(dfa),paste(region,"_<=183m",sep='')]))
  temp2 = t(matrix(temp[colnames(dfa),paste(region,"_184-550m",sep='')]))
  temp3 = t(matrix(temp[colnames(dfa),paste(region,"_>=550m",sep='')]))
...
write.csv(cov.tbl, file=paste("C:/MY_FILES/ANALYSES/Growth analysis - California current/mean-length-at-age/",region,"/",region,"_",age,"_covariates table.csv",sep=''))
